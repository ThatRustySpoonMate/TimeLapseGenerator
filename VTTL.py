...
kept_frame_count = 0  # Counter
frames = []  # Counter
first_frame = None  # Counter


# Split video into frames and store them depending on the mode
def extract_frames_from_source(
    src_path, skip_n_frames, mode=1
):  # mode 1: RAM, mode 2: HDD
...
if __name__ == "__main__":
    if not os.path.exists(source_file_path):
        print(
            "Unable to open '{}'\nPlease check the path and try again.".format(
                source_file_path
            )
        )
        quit()
    print(
        "Conversion begun in {} mode.".format(
            "direct access"
            if hardware_mode == 0
            else "RAM buffered"
            if hardware_mode == 1

...
""" Uncomment for file-based store and loading (For low ram applications)
print("CONVERTING.")
print("Step 1: |", end='')
...
def FrameCapture(path, skip_n_frames, outfile):
    ...
    while success:
        ...
        if count % skip_n_frames == 0:
            ...
            if( (pic_count / target_frames * 100) - step_1_progress > 5):
                step_1_progress = step_1_progress + 5
                print("o", end='')
        count +=1
...
if __name__ == '__main__':
    ...
    # Reconstruct frames into video
    
    print("Step 2: |", end='')
    step_2_progress = 0
    # Method 1 - Issue with GLOB
    #source_input = source.get_input(['./temp/*.jpg'], fps=24, deflicker=10)
    #output.create_outputs(source_input, 'test_movie', verbose=False)  

    # Method 2
    image_folder = temp_dir
    video_name = dest_vid_name

    images = [img for img in os.listdir(temp_dir) if img.endswith(".jpg")]
    frame = cv2.imread(os.path.join(temp_dir, images[0]))
    height, width, layers = frame.shape

    video = cv2.VideoWriter(dest_vid_name, 0, 25, (width,height))
    img_count = 0
    ...
    for image in images:
        ...
        if( (img_count / pic_count * 100) - step_2_progress > 5):
                step_2_progress = step_2_progress + 5
                print("o", end='')
